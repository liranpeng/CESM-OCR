...
!         parts:
!             - global_comm, which only has processes spawned by cesm.exe, and
!             - a separate communicator made up of MPI processes spawned by 
!                   separate CRM executable
...
!bloss    call shr_mpi_chkerr(ierr,subname//' mpi_comm_dup')
    ...
    if(mype.eq.0) then
       ...
       call shr_mpi_commsize(MPI_COMM_WORLD, numpes, ' cime_pre_init1b')
       write(logunit,*) 'bloss: MPI_COMM_WORLD size = ', numpes
       call shr_mpi_commsize(global_comm, numpes, ' cime_pre_init1c')
       write(logunit,*) 'bloss: global_comm size = ', numpes
    end if
    !bloss: End of MPI Split related code...
