module camsrfexch
  ...
  public cam_export
  public gcm2crm_alloc
  public crm2gcm_alloc  
...
!===============================================================================
  ...
  !===============================================================================
  subroutine gcm2crm_alloc( crm_orc_in )

    ! Allocate space for the atmosphere to surface data type. And initialize
    ! the values.

    ! ARGUMENTS:
    type(crm_in_t), pointer :: crm_orc_in(:)    ! Atmosphere to surface input

    ! LOCAL VARIABLES:
    integer :: c            ! chunk index
    integer :: ierror       ! Error code
    character(len=*), parameter :: sub = 'gcm2crm_alloc'
    !----------------------------------------------------------------------- 
    do c = begchunk,endchunk
       crm_orc_in(c)%cld(:)     = 0._r8
    end do
  end subroutine gcm2crm_alloc

  subroutine crm2gcm_alloc( crm_orc_out )

    ! Allocate space for the atmosphere to surface data type. And initialize
    ! the values.

    ! ARGUMENTS:
    type(crm_out_t), pointer :: crm_orc_out(:)    ! Atmosphere to surface input

    ! LOCAL VARIABLES:
    integer :: c            ! chunk index
    integer :: ierror       ! Error code
    character(len=*), parameter :: sub = 'crm2gcm_alloc'
    !----------------------------------------------------------------------- 
    do c = begchunk,endchunk
       crm_orc_out(c)%cld(:)     = 0._r8
    end do
  end subroutine crm2gcm_alloc
  ...
  subroutine gcm2crm_deallocate(crm_orc_in)

    type(crm_in_t), pointer :: crm_orc_in(:)    ! Atmosphere to surface input
    !----------------------------------------------------------------------- 

    if(associated(crm_orc_in)) then
       deallocate(crm_orc_in)
    end if
    nullify(crm_orc_in)

  end subroutine gcm2crm_deallocate

  subroutine crm2gcm_deallocate(crm_orc_out)

    type(crm_out_t), pointer :: crm_orc_out(:)    ! Atmosphere to surface input
    !----------------------------------------------------------------------- 

    if(associated(crm_orc_out)) then
       deallocate(crm_orc_out)
    end if
    nullify(crm_orc_out)

  end subroutine crm2gcm_deallocate
