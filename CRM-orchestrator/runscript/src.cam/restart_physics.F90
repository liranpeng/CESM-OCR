...
#ifdef CRM
#ifdef ORCHESTRATOR
    !bloss(TODO): If 3D CRM fields are not passed from separate CRM
    !  executables during each GCM timestep, a routine should be called
    !  to pass them here since they will be needed for a clean restart.
    !  We should also make sure isorchestrated is also saved.
#endif
#endif
