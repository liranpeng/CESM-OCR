...
!
   ...
   integer,dimension(35) :: blocklen, types
   integer(kind=MPI_ADDRESS_KIND)               ::  base
   integer(kind=MPI_ADDRESS_KIND),dimension(35) :: displacement,displ  
   integer :: g2cMPItype
#ifdef ORCHESTRATOR
    integer :: dest,ierr,nflat,fcount,chnksz,kk,ll
...
! ifdef required because of loadaer
       ...
       ! sending required inputs...
         if (state_loc%crmrank0(i) .ne. -1) then ! is this column coupled to an external CRM?
          g2c%cld(1:pver) = cld(i,:)
          g2c%cldtop(1:pver) = cldtop(i,:)
          g2c%gicewp(1:pver) = gicewp(i,:)
          g2c%gliqwp(1:pver) = gliqwp(i,:)
          g2c%mctot(1:pver) = mctot(i,:)
          g2c%mcup(1:pver) = mcup(i,:)
          g2c%mcdn(1:pver) = mcdn(i,:)
          g2c%mcuup(1:pver) = mcuup(i,:)
          g2c%mcudn(1:pver) = mcudn(i,:)
          g2c%spqc(1:pver) = spqc(i,:)
          g2c%spqi(1:pver) = spqi(i,:)
          g2c%spqs(1:pver) = spqs(i,:)
          g2c%spqg(1:pver) = spqg(i,:)
          g2c%spqr(1:pver) = spqr(i,:)
          g2c%mu_crm(1:pver) = mu_crm(i,:)
          g2c%md_crm(1:pver) = md_crm(i,:)
          g2c%du_crm(1:pver) = du_crm(i,:)
          g2c%eu_crm(1:pver) = eu_crm(i,:)
          g2c%ed_crm(1:pver) = ed_crm(i,:)
          g2c%tkez(1:pver) = tkez(i,:)
          g2c%tkesgsz(1:pver) = tkesgsz(i,:)
          g2c%tk_crm(1:pver) = tk_crm(i,:)
          g2c%flux_u(1:pver) = flux_u(i,:)
          g2c%flux_v(1:pver) = flux_v(i,:)
          g2c%flux_qt(1:pver) = flux_qt(i,:)
          g2c%fluxsgs_qt(1:pver) = fluxsgs_qt(i,:)
          g2c%flux_qp(1:pver) = flux_qp(i,:)
          g2c%precflux(1:pver) = precflux(i,:)
          g2c%qt_ls(1:pver) = qt_ls(i,:)
          g2c%qt_trans(1:pver) = qt_trans(i,:)
          g2c%qp_trans(1:pver) = qp_trans(i,:)
          g2c%qp_fall(1:pver) = qp_fall(i,:)
          g2c%qp_evp(1:pver) = qp_evp(i,:)
          g2c%qp_src(1:pver) = qp_src(i,:)
          g2c%t_ls(1:pver) = t_ls(i,:)
          call MPI_GET_ADDRESS(g2c%cld, displacement(1), ierr)
          call MPI_GET_ADDRESS(g2c%cldtop, displacement(2), ierr)
          call MPI_GET_ADDRESS(g2c%gicewp, displacement(3), ierr)
          call MPI_GET_ADDRESS(g2c%gliqwp, displacement(4), ierr)
          call MPI_GET_ADDRESS(g2c%mctot, displacement(5), ierr)
          call MPI_GET_ADDRESS(g2c%mcup, displacement(6), ierr)
          call MPI_GET_ADDRESS(g2c%mcdn, displacement(7), ierr)
          call MPI_GET_ADDRESS(g2c%mcuup, displacement(8), ierr)
          call MPI_GET_ADDRESS(g2c%mcudn, displacement(9), ierr)
          call MPI_GET_ADDRESS(g2c%spqc, displacement(10), ierr)
          call MPI_GET_ADDRESS(g2c%spqi, displacement(11), ierr)
          call MPI_GET_ADDRESS(g2c%spqs, displacement(12), ierr)
          call MPI_GET_ADDRESS(g2c%spqg, displacement(13), ierr)
          call MPI_GET_ADDRESS(g2c%spqr, displacement(14), ierr)
          call MPI_GET_ADDRESS(g2c%mu_crm, displacement(15), ierr)
          call MPI_GET_ADDRESS(g2c%md_crm, displacement(16), ierr)
          call MPI_GET_ADDRESS(g2c%du_crm, displacement(17), ierr)
          call MPI_GET_ADDRESS(g2c%eu_crm, displacement(18), ierr)
          call MPI_GET_ADDRESS(g2c%ed_crm, displacement(19), ierr)
          call MPI_GET_ADDRESS(g2c%tkez, displacement(20), ierr)
          call MPI_GET_ADDRESS(g2c%tkesgsz, displacement(21), ierr)
          call MPI_GET_ADDRESS(g2c%tk_crm, displacement(22), ierr)
          call MPI_GET_ADDRESS(g2c%flux_u, displacement(23), ierr)
          call MPI_GET_ADDRESS(g2c%flux_v, displacement(24), ierr)
          call MPI_GET_ADDRESS(g2c%flux_qt, displacement(25), ierr)
          call MPI_GET_ADDRESS(g2c%fluxsgs_qt, displacement(26), ierr)
          call MPI_GET_ADDRESS(g2c%flux_qp, displacement(27), ierr)
          call MPI_GET_ADDRESS(g2c%precflux, displacement(28), ierr)
          call MPI_GET_ADDRESS(g2c%qt_ls, displacement(29), ierr)
          call MPI_GET_ADDRESS(g2c%qt_trans, displacement(30), ierr)
          call MPI_GET_ADDRESS(g2c%qp_trans, displacement(31), ierr)
          call MPI_GET_ADDRESS(g2c%qp_fall, displacement(32), ierr)
          call MPI_GET_ADDRESS(g2c%qp_evp, displacement(33), ierr)
          call MPI_GET_ADDRESS(g2c%qp_src, displacement(34), ierr)
          call MPI_GET_ADDRESS(g2c%t_ls, displacement(35), ierr)

          base = displacement(1)
          do jj=1,35
            blocklen(jj)=pver
            displ(jj) = displacement(jj) - base
            types(jj) = MPI_REAL8
          end do
          g2cMPItype = -99999
          call MPI_TYPE_CREATE_STRUCT(35, blocklen, displ, types, g2cMPItype, ierr)
          call MPI_TYPE_COMMIT(g2cMPItype, ierr)

          dest = npes+state%crmrank0(i)
          dest = 52
          write (iulog,*) 'MDEBUG YO preparing to send CRM inputs to Orchestrator.'
          call MPI_Send(lchnk                    ,1   ,MPI_INTEGER,dest,9001,MPI_COMM_WORLD,ierr)
          call MPI_Send(i                        ,1   ,MPI_INTEGER,dest,9002,MPI_COMM_WORLD,ierr)
          call MPI_Send(state_loc%t(i,:)         ,pver,MPI_REAL8  ,dest,9003,MPI_COMM_WORLD,ierr)
          call MPI_Send(state_loc%q(i,:,1)       ,pver,MPI_REAL8  ,dest,9004,MPI_COMM_WORLD,ierr)
          call MPI_Send(state_loc%q(i,:,ixcldliq),pver,MPI_REAL8  ,dest,9005,MPI_COMM_WORLD,ierr)
          call MPI_Send(state_loc%q(i,:,ixcldice),pver,MPI_REAL8  ,dest,9006,MPI_COMM_WORLD,ierr)
          call MPI_Send(ul(:)                    ,pver,MPI_REAL8  ,dest,9007,MPI_COMM_WORLD,ierr)
          call MPI_Send(vl(:)                    ,pver,MPI_REAL8  ,dest,9008,MPI_COMM_WORLD,ierr)
          call MPI_Send(state_loc%ps(i)          ,1   ,MPI_REAL8  ,dest,9009,MPI_COMM_WORLD,ierr)
          call MPI_Send(state_loc%pmid(i,:)      ,pver,MPI_REAL8  ,dest,9010,MPI_COMM_WORLD,ierr)
          call MPI_Send(state_loc%pdel(i,:)      ,pver,MPI_REAL8  ,dest,9011,MPI_COMM_WORLD,ierr)
          call MPI_Send(state_loc%phis(i)        ,1   ,MPI_REAL8  ,dest,9012,MPI_COMM_WORLD,ierr)
          call MPI_Send(state_loc%zm(i,:)        ,pver,MPI_REAL8  ,dest,9013,MPI_COMM_WORLD,ierr)
          call MPI_Send(state_loc%zi(i,:)        ,pver,MPI_REAL8  ,dest,9014,MPI_COMM_WORLD,ierr)
          call MPI_Send(ztodt                    ,1   ,MPI_REAL8  ,dest,9015,MPI_COMM_WORLD,ierr)
          call MPI_Send(g2cMPItype               ,1   ,MPI_INTEGER,dest,9016,MPI_COMM_WORLD,ierr)
          call MPI_Send(g2c%cld(1)               ,1   ,g2cMPItype ,dest,9017,MPI_COMM_WORLD,ierr)


  write (iulog,*) 'sending lchnk=',lchnk,dest,npes,state%crmrank0(i),i
  write (iulog,*) 'sending i=',i,dest,npes,state%crmrank0(i),i
  write (iulog,*) 'sending t=',state_loc%t(i,:),dest,npes,state%crmrank0(i),i
  write (iulog,*) 'sending g2cMPItype=',g2cMPItype,dest,npes,state%crmrank0(i),i
  write (iulog,*) 'Finish sending'
          ...
          !call MPI_Send(precc(i),1,MPI_REAL8,dest,9017,MPI_COMM_WORLD,ierr)
          !call MPI_Send(precl(i),1,MPI_REAL8,dest,9018,MPI_COMM_WORLD,ierr)
          !call MPI_Send(precsc(i),1,MPI_REAL8,dest,9019,MPI_COMM_WORLD,ierr)
          !call MPI_Send(precsl(i),1,MPI_REAL8,dest,9020,MPI_COMM_WORLD,ierr)
          !call MPI_Send(cltot(i),1,MPI_REAL8,dest,9021,MPI_COMM_WORLD,ierr)
          !call MPI_Send(clhgh(i),1,MPI_REAL8,dest,9022,MPI_COMM_WORLD,ierr)
          !call MPI_Send(clmed(i),1,MPI_REAL8,dest,9023,MPI_COMM_WORLD,ierr)
          !call MPI_Send(cllow(i),1,MPI_REAL8,dest,9024,MPI_COMM_WORLD,ierr)
          !call MPI_Send(cld(i,:),pver,MPI_REAL8,dest,9025,MPI_COMM_WORLD,ierr)

          !call MPI_Send(prectend(i),1,MPI_REAL8,dest,9061,MPI_COMM_WORLD,ierr)
          !call MPI_Send(precstend(i),1,MPI_REAL8,dest,9062,MPI_COMM_WORLD,ierr)
          !call MPI_Send(cam_in%ocnfrac(i),1,MPI_REAL8,dest,9063,MPI_COMM_WORLD,ierr)
          !call MPI_Send(wnd,1,MPI_REAL8,dest,9064,MPI_COMM_WORLD,ierr)
          !call MPI_Send(tau00,1,MPI_REAL8,dest,9065,MPI_COMM_WORLD,ierr)
          !call MPI_Send(bflx,1,MPI_REAL8,dest,9066,MPI_COMM_WORLD,ierr)
          !call MPI_Send(fluxu0,1,MPI_REAL8,dest,9067,MPI_COMM_WORLD,ierr)
          !call MPI_Send(fluxv0,1,MPI_REAL8,dest,9068,MPI_COMM_WORLD,ierr)
          !call MPI_Send(fluxt0,1,MPI_REAL8,dest,9069,MPI_COMM_WORLD,ierr)
          !call MPI_Send(fluxq0,1,MPI_REAL8,dest,9070,MPI_COMM_WORLD,ierr)
          !call MPI_Send(taux_crm(i),1,MPI_REAL8,dest,9071,MPI_COMM_WORLD,ierr)
          !call MPI_Send(tauy_crm(i),1,MPI_REAL8,dest,9072,MPI_COMM_WORLD,ierr)
          !call MPI_Send(z0m(i),1,MPI_REAL8,dest,9073,MPI_COMM_WORLD,ierr)
          !call MPI_Send(timing_factor(i),1,MPI_REAL8,dest,9074,MPI_COMM_WORLD,ierr)
          !call MPI_Send(qtotcrm(i,:),pver,MPI_REAL8,dest,9075,MPI_COMM_WORLD,ierr)
#ifdef m2005
          ...
          !call MPI_Recv(ptend_loc%q(i,:,1),pver,MPI_REAL8,MPI_ANY_SOURCE,8001,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(ptend_loc%q(i,:,ixcldliq),pver,MPI_REAL8,MPI_ANY_SOURCE,8002,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(ptend_loc%q(i,:,ixcldice),pver,MPI_REAL8,MPI_ANY_SOURCE,8003,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(ptend_loc%s(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8004,MPI_COMM_WORLD,status,ierr)
         ...
         ! unit test.
! Liran Start Here   ======================================================
          !call MPI_Recv(flattened_crm_inout(:),pver,MPI_REAL8,MPI_ANY_SOURCE,8005,MPI_COMM_WORLD,status,ierr)

          !call MPI_Recv(cld(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8006,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(cldtop(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8007,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(gicewp(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8008,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(gliqwp(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8009,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(mctot(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8010,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(mcup(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8011,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(mcdn(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8012,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(mcuup(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8013,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(mcudn(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8014,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(spqc(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8015,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(spqi(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8016,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(spqs(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8017,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(spqg(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8018,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(spqr(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8019,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(mu_crm(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8020,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(md_crm(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8021,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(du_crm(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8022,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(eu_crm(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8023,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(ed_crm(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8024,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(tkez(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8025,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(tkesgsz(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8026,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(tk_crm(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8027,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(flux_u(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8028,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(flux_v(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8029,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(flux_qt(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8030,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(fluxsgs_qt(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8031,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(flux_qp(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8032,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(qt_ls(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8033,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(qt_trans(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8034,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(qp_trans(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8035,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(qp_fall(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8036,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(qp_evp(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8037,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(qp_src(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8038,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(t_ls(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8039,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(qtotcrm(i,:),pver,MPI_REAL8,MPI_ANY_SOURCE,8040,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(precc,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8041,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(precl,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8042,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(precsc,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8043,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(precsl,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8044,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(cltot,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8045,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(clhgh,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8046,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(clmed,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8047,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(cllow,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8048,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(prectend,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8049,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(precstend,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8050,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(ocnfrac,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8051,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(wnd,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8052,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(tau00,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8053,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(bflx,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8054,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(fluxu0,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8055,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(fluxv0,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8056,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(fluxt0,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8057,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(fluxq0,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8058,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(taux_crm,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8059,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(z0m,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8060,MPI_COMM_WORLD,status,ierr)
          !call MPI_Recv(timing_factor,1,pver,MPI_REAL8,MPI_ANY_SOURCE,8061,MPI_COMM_WORLD,status,ierr)
! Liran End Here   ======================================================
#endif
